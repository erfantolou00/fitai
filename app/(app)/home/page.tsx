...
export default function HomePage() {
  ...
  if (error || !data) {
    return <Alert variant="danger">{error ?? 'خطا'}</Alert>;
  }

  const firstName = data.profile?.full_name?.split(' ')[0] ?? 'ورزشکار';
  const { stats, fitness, latestPlan, tips, billing } = data;

  const trialDaysLeft = billing?.freeTrialExpiresAt
    ? Math.max(
        0,
        Math.ceil(
          (new Date(billing.freeTrialExpiresAt).getTime() - Date.now()) /
            (1000 * 60 * 60 * 24)
        )
      )
    : null;
  ...
  return (
    <>
      {trialDaysLeft !== null && trialDaysLeft <= 14 && (
        <Alert variant={trialDaysLeft <= 3 ? 'warning' : 'info'} className="mb-6">
          {trialDaysLeft > 0
            ? `${trialDaysLeft} روز از دوره رایگان یک‌ماهه برنامه اول باقی مانده`
            : 'دوره رایگان تمام شده — برای برنامه جدید پرداخت لازم است'}
        </Alert>
      )}
      <div className="mb-8">
        <h1 className="text-2xl font-bold m-0">سلام {firstName} 👋</h1>
        ...
        <Card variant="success" className="mb-6">
          <div className="flex justify-between items-start gap-3 mb-3">
            <CardTitle className="text-primary-dark">
              {latestPlan.isDefault ? 'برنامه اصلی' : 'آخرین برنامه'}
            </CardTitle>
...
function TipCard({
  ...
  return (
    <Card variant={variant} padding="md">
      ...
      <p
        className={cn(
          'font-semibold text-sm m-0 mb-1',
          variant === 'success' ? 'text-primary-dark' : 'text-info-dark'
        )}
      >
        {title}
      </p>
